...
const pageParam: number = Number(
  new URLSearchParams(window.location.search).get("page") || "1",
);
let currentPage: number = pageParam >= 1 ? pageParam : 1;
...
function renderPagination(page: number, totalPages: number): void {
  ...
  const previousDisabled: string = page <= 1 ? "disabled" : "";
  const nextDisabled: string = page >= totalPages ? "disabled" : "";
...
async function loadList(): Promise<void> {
  ...
  const url: string = `${BASE_URL}/${getEndpoint()}?api_key=${API_KEY}&language=fr-FR&page=${currentPage}`;
  try {
