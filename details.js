 ...
 */
function setupFavoriteButton(mediaId, type, title, poster) {
    const container = document.getElementById("favorite-action");
    if (!container || !isUserConnected)
        return;
    const updateUI = () => {
        const favs = JSON.parse(localStorage.getItem("favorites") || "[]");
        const isFav = favs.some((f) => f.id === mediaId && f.type === type);
        container.innerHTML = `
    ...
    `;
        document.getElementById("btn-fav")?.addEventListener("click", () => {
            ...
            if (isFav) {
                current = current.filter((f) => !(f.id === mediaId && f.type === type));
            }
            else {
